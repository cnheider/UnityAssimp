...
namespace Editor {
    ...
    public static class ImportAssimpSkinnedCombine {
        ...
        [MenuItem(Assimp._MenuPath +"ImportSkinnedOptimize")]
        static void Init() {
        ...
        public class AssimpMesh {
            ...
            public void BuilSkin() {
                for (var i = 0; i < this._BoneVertexList.Count; i++) {
                    ...
                    this._max_bones = Math.Max(this._max_bones, vertex._Numbones);

                    Trace("----------");
                    Trace(" ");
                    Trace(string.Format(" vertex:{0} num Bones:{1} force {2}", i, vertex._Numbones, len));
            ...
            public void Build() {
                ...
                switch (this._max_bones) {
                    ...
                    default: {
                            Debug.Log("????? set " + this._max_bones.ToString() + " bones ???");
                            this._MeshRenderer.quality = SkinQuality.Bone1;
                            break;
                        }
                }
        ...
        private static void ConfigureForAtlas(string texturePath) {
            var texImport = AssetImporter.GetAtPath(texturePath) as TextureImporter;
            var tiSettings = new TextureImporterSettings();
        ...
        private static Mesh CombineInstance(GameObject root, GameObject obj, string path) {
            var smRenderers = obj.GetComponentsInChildren<SkinnedMeshRenderer>();
            ...
            var r = obj.AddComponent<SkinnedMeshRenderer>();
            ...
            var assetPath = AssetDatabase.GenerateUniqueAssetPath(path + "texture.asset");
